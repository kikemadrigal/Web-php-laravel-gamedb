...
class UserRepository{
	public static function obtenerTodos(){
		...
		PdoClient::abrirConexion();
		$conexion=PdoClient::obtenerConexion();
		if(isset($conexion)){
			try{
				...
				//Si sale cero el bloque no se ejecuta
				if(count($resultado)){
					foreach($resultado as $fila){
						$usuario=new Usuario($fila['idusuario']);
						$usuario->setNombreUsuario($fila['nombreusuario']);
						$usuario->setClaveusuario($fila['claveusuario']);
						$usuario->setNivelAccesoUsuario($fila['nivelaccesousuario']);
						$usuario->setCorreoUsuario($fila['email']);
						$usuario->setNombrerealUsuario($fila['nombrerealusuario']);
						$usuario->setAppelidosUsuario($fila['apellidosusuario']);
						$users[]=$usuario;
					}
				}else{
					print "No hay users";
